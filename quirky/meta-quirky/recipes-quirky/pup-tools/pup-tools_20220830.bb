...
# 20220903 bacon 4.5.0.2

# 20220829 PR_NUM is defined in local.conf, currently r9
PR = "r${@int(PR_NUM) + 1}"
...
SRC_URI[md5sum] = "d381fe1917cac5c634fe0c87fee8c383"
SRC_URI[sha1sum] = "4771c4cd00745c4ec2b420ffec9e555aedf24f74"
SRC_URI[sha256sum] = "4824a844e6dd6a8b138a8e70be93ce9bd805a5507d469409a1a2df1daf69f9eb"
...
do_compile () {
 ...
 cd bacon
  ...
  do
   bacon -d build -c "${CC}" -o "${CFLAGS}" -l "${LDFLAGS}" ${aF}.bac
   if [ $? -eq 0 ];then
    cp -a -f build/${aF} ./
   fi
   rm -f build/*
